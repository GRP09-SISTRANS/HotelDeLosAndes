...
public class SuperServicio {
    ...
    /*
     * - Inicialmente, se construye una consulta SQL dinámica basada en el valor de `tipoAgrupamiento`.
     * - Si `tipoAgrupamiento` es "Nada", la consulta selecciona el ID del consumo, el ID del servicio, el ID del usuario, y los nombres y apellidos del usuario.
     * - Si `tipoAgrupamiento` es "Fecha", la consulta selecciona la fecha y cuenta el número de consumos.
     * - Si `tipoAgrupamiento` es "Frecuencia de Consumo", la consulta selecciona la frecuencia de consumo y cuenta el número de usuarios.
     * - Si `tipoAgrupamiento` es "Cliente", la consulta selecciona el ID del usuario, los nombres y apellidos del usuario, y cuenta el número de consumos.
     * - Si `tipoAgrupamiento` es "Fecha y Cliente", la consulta selecciona la fecha, el número de consumos, el ID del servicio, el ID del usuario, y los nombres y apellidos del usuario.
     * - Luego, la consulta se ejecuta en la base de datos y los resultados se almacenan en una lista de objetos `SuperObjeto`.
     * - Cada objeto `SuperObjeto` almacena los datos relevantes dependiendo del valor de `tipoAgrupamiento`. Por ejemplo, si `tipoAgrupamiento` es "Fecha y Cliente", el objeto `SuperObjeto` almacenará la fecha, el número de consumos, el ID del servicio, el ID del usuario, y los nombres y apellidos del usuario. Si `tipoAgrupamiento` es "Cliente", el objeto `SuperObjeto` almacenará el ID del usuario, los nombres y apellidos del usuario, y el número de consumos. Si `tipoAgrupamiento` es "Frecuencia de Consumo", el objeto `SuperObjeto` almacenará la frecuencia de consumo y el número de usuarios.
     * - Finalmente, la función devuelve la lista de objetos `SuperObjeto`.
     */
